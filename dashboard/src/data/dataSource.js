...
export async function getDrrmoData() {
  ...
  return {
    stats: [
      { key: 'advisories', label: 'Active Advisories',   value: d.activeAdvisories,           accent: 'alert'   },
      { key: 'atRisk',     label: 'Barangays at Risk',   value: d.barangaysAtRisk,            accent: 'fail'    },
      { key: 'incidents',  label: 'Incidents Today',     value: d.incidentsToday,             accent: 'amber',  sub: `${d.activeIncidents} still open` },
      { key: 'teams',      label: 'Teams Deployed',      value: d.teamsDeployed,              accent: 'neutral' },
      { key: 'affected',   label: 'Residents Affected',  value: mock.SCENARIO.residentsAffected, accent: 'fail', sub: `${mock.SCENARIO.familiesAffected} families` },
      { key: 'pwdSenior',  label: 'PWD / Senior Assisted', value: mock.PWD_SENIOR_ASSISTED,   accent: 'violet'  },
    ],
...
export async function getLguData() {
  ...
  return {
    stats: [
      { key: 'occupancy',  label: 'EC Occupancy',        value: d.totalOccupied, unit: `/${d.totalCapacity}`, accent: 'neutral', sub: `${d.activeCenterCount} active centers` },
      { key: 'families',   label: 'Affected Families',   value: mock.SCENARIO.familiesAffected, accent: 'pass'  },
      { key: 'evacuees',   label: 'Total Evacuees',      value: mock.SCENARIO.residentsAffected, accent: 'amber' },
      { key: 'barangays',  label: 'Barangays Affected',  value: d.barangaysAtRisk,              accent: 'fail'  },
      { key: 'supplies',   label: 'Supplies Left',       value: mock.SUPPLIES_REMAINING_PCT, unit: '%', accent: 'violet' },
      { key: 'pwdSenior',  label: 'PWD / Senior Assisted', value: mock.PWD_SENIOR_ASSISTED,  accent: 'alert' },
    ],
